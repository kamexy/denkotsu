...
- **前提**: Phase 3（広告・アフィリエイト基盤）が完了している
- **コンテンツ**: 複線図練習、欠陥判定クイズ、40分タイムライン練習
...
- 動画は品質と運用コストの観点から採用しない。
- 実技トップは、以下3導線を常設して学習ループを作る。
  1. 複線図ステップ練習（候補13問）
  2. 欠陥判定クイズ（30問）
  3. 40分タイムライン練習
...
- 候補13問のチェック進捗（完了数/達成率）を表示する。
- 各導線に即遷移できるカードUIを配置する。
- 進捗はローカル保存し、再訪時に継続できる。
...
1. **実技トップ導線**: 複線図 / 欠陥判定 / タイムラインの3導線が動作する
2. **複線図練習**: 13問すべてでステップ式ガイドが動作する
3. **欠陥判定**: 30問のクイズが動作し、欠陥問題では画像上に欠陥箇所がマーキングされる
4. **タイムライン**: 40分タイマー、フェーズ手動切替、実績表示が動作する
5. **アクセス制御**: 課金制御なし（全ユーザーが閲覧可能）
